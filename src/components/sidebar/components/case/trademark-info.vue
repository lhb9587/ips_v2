        ...
        >
          <tbody>
            ...
            <tr v-if="!caseInfo.usAgency">
              ...
              <td>{{ toDisplay(showTmDesignDeclare ? caseInfo.tmDesignDeclare : "") }}</td>
            </tr>
            ...
            <tr v-if="showParentAppDateBase">
              ...
              <td v-if="showRejectReviewBaseRegNumber">{{ caseInfo.regNumber }}</td>
            </tr>
            ...
            <tr v-if="showRegDate">
              ...
              <td v-if="showJoinReviewApprovalNo">{{ caseInfo.approvalNo }}</td>
            </tr>
            ...
            <tr v-if="!caseInfo.usAgency">
              ...
              <td v-if="showSolidFlag">{{ caseInfo.ifSolidTm == "1" ? "是" : "否" }}</td>
            </tr>
            ...
            <tr v-if="showChangeNameAddr">
              ...
              <td>{{ toDisplay(showChangeCertLang ? (caseInfo.appCertFileIsCn ? "是" : "否") : "") }}</td>
            </tr>
            <tr v-if="showChangeNameAddr && caseInfo.changeType !== '变更地址'">
              <th scope="row">变更证明文件（英文） :</th>
              <td>{{ toDisplay(caseInfo.appCertFileIsCn === "否" ? "-" : "") }}</td>
...
export default {
  ...
  data() {
    return {
      ...
      parentAppDateTypes: ["撤销三年停止使用申请", "异议", "异议答辩", "撤三答辩（提供使用证明）", "无效宣告复审", "撤销商标复审", "不予注册复审", "无效宣告申请", "参加不予注册复审", "撤销复审答辩", "无效宣告答辩"],
      officeDocTypes: ["撤销复审答辩", "异议答辩", "无效宣告答辩", "国际注册驳回复审", "注册驳回复审", "不予注册复审", "无效宣告复审", "撤销商标复审", "撤三答辩（提供使用证明）", "参加不予注册复审"],
      submitDateTypes: ["答复临时驳回/审查意见（境外）", "提供使用声明/证据（境外）", "马德里商标转国内注册", "出具商标注册证明", "不予注册复审", "商标注册", "分割申请", "异议", "变名变址", "转让/移转", "续展", "删减商品项目", "变更注册申请代理机构", "更正商标申请事项", "注册驳回复审", "国际注册驳回复审", "无效宣告复审", "撤销商标复审", "异议答辩", "撤三答辩(提供使用证明)", "撤销通用名称答辩", "参与不予注册复审", "无效宣告答辩", "撤销复审答辩", "撤销三年停止使用申请", "撤销成为通用名称注册商标", "无效宣告申请", "撤回商标评审", "行政复议", "许可备案", "补发商标注册证", "补发商标变转续证明", "商标注销", "撤回商标申请"],
    };
  },
  computed: {
    caseType() {
    ...
    goodsLabel() {
      return this.caseType === "商标注销" ? "注销商品服务项目\n(商品/服务项目)" : "商品/服务项目";
    },
    showMadridToCn() {
      return !this.caseInfo.usAgency && this.caseType === "马德里商标转国内注册";
    },
    showPriorityProof() {
      return !this.caseInfo.usAgency && this.caseType === "出具优先权证明文件";
    },
    showParentAppDateBase() {
      return !this.caseInfo.usAgency && this.parentAppDateTypes.includes(this.caseType);
    },
    showRejectReviewBaseRegNumber() {
      return !this.caseInfo.usAgency && ["国际注册驳回复审", "注册驳回复审"].includes(this.caseType);
    },
    rejectReviewLabel() {
      return this.caseType === "注册驳回复审" ? "原申请号/国际注册号" : "原商标申请号/国际注册号";
    },
    showTmDesignDeclare() {
      return !this.caseInfo.usAgency && this.caseType !== "变名变址";
    },
    showRefBasic() {
      return !this.caseInfo.usAgency && ["撤销复审答辩", "异议答辩", "无效宣告申请", "无效宣告答辩"].includes(this.caseType);
    },
    showRefRegNumberExt() {
      return !this.caseInfo.usAgency && ["撤销复审答辩", "异议答辩", "无效宣告答辩", "不予注册复审", "无效宣告复审", "撤销商标复审"].includes(this.caseType);
    },
    showAppNumberExt() {
      return !this.caseInfo.usAgency && ["撤销复审答辩", "异议答辩"].includes(this.caseType);
    },
    showOfficeDocGroup() {
      return !this.caseInfo.usAgency && this.officeDocTypes.includes(this.caseType);
    },
    showGeneralSubmitDate() {
      return !this.caseInfo.usAgency && this.submitDateTypes.includes(this.caseType);
    },
    showApprovalSet() {
      return !this.caseInfo.usAgency && (["马德里商标转国内注册", "异议", "异议答辩", "分割申请", "不予注册复审"].includes(this.caseType) || this.trademarkList.includes(this.caseType));
    },
    ...
    showWithdrawRegNo() {
      return !this.caseInfo.usAgency && ["撤回商标申请", "撤回商标评审"].includes(this.caseType) && this.caseInfo.changeType === "转让/移转商标";
    },
    showWithdrawOriginalRegNo() {
      return !this.caseInfo.usAgency && ["撤回商标申请", "撤回商标评审"].includes(this.caseType) && this.caseInfo.changeType !== "转让/移转商标";
    },
    withdrawRegNoLabel() {
      return ["撤销三年不使用注册商标", "转让/移转商标"].includes(this.caseInfo.changeType) ? "商标号" : "原商标号";
    },
    showEarlyStop() {
      return !this.caseInfo.usAgency && this.caseType === "许可备案提前终止" && ![62, 48].includes(this.caseTypeId);
    },
    showRegBase() {
      return !this.caseInfo.usAgency && this.caseType !== "出具优先权证明文件" && this.caseType !== "异议" && this.caseType !== "异议答辩" && ![62, 48].includes(this.caseTypeId);
    },
    showRegDate() {
      return this.showRegBase && !["不予注册复审", "异议答辩", "注册驳回复审", "国际注册驳回复审", "撤回商标申请", "撤回商标评审", "参加不予注册复审"].includes(this.caseType);
    },
    ...
    showValidPeriod() {
      return this.showRegBase && !["参加不予注册复审", "不予注册复审", "商标注销", "异议答辩", "注册驳回复审", "国际注册驳回复审", "撤回商标申请", "撤回商标评审"].includes(this.caseType);
    },
    showGoodsCheckFile() {
    ...
    showMemberRule() {
      return !this.caseInfo.usAgency && ["2", "3", 2, 3].includes(this.caseInfo.tmType);
    },
    showMemberList() {
    ...
    showDetectAttach() {
      return this.showDetectAbility && this.caseInfo.isAppWithDetectAbility === "是";
    },
    showColorVoice() {
      return !this.caseInfo.usAgency && this.caseType !== "变更注册申请代理机构";
    },
