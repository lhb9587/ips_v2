<script>
import ConfigContainer from "@/components/common/charts/config-container";
import { getChartData } from "@/api/dashboard";

export default {
  name: "AverageYearEmployees",
  components: { ConfigContainer },
  props: {
    cardDesc: {
      type: String,
      default: ""
    }
  },
  data() {
    return {
      isEnlarge: false,
      cardData: {},
      mouseInner: false,
      showImage: true,
    };
  },
  methods: {
    enlarge() {
      this.isEnlarge = true;
    },
    close() {
      this.isEnlarge = false;
    },
    fetchData() {
      const params = {
        isSeniorPanel: 1,
        panelId: 4034,
      };
      getChartData(params).then((res) => {
        this.cardData = res.data.chartDatas;
      });
    },
    formatDuration(months) {
      if (!months) {
        return "0";
      }
      const years = Math.floor(months / 12);
      const remainingMonths = months % 12;
      let result = "";
      if (years > 0) {
        result += years + "年";
      }
      if (remainingMonths > 0) {
        result += remainingMonths + "个月";
      }
      // 如果是0年0个月，可以输出"0个月"或者其他
      if (years === 0 && remainingMonths === 0) {
        result = "0";
      }
      return result;
    },
    handleMouseLeave(event) {
      if (event.relatedTarget && event.relatedTarget.classList) {
        const contentClass = [
          "el-popover--plain",
          "el-popover",
          "is-light",
          "el-popper",
          "el-popper__arrow"
        ];
        let isLeave = true;
        contentClass.forEach((item) => {
          if (event.relatedTarget.classList.contains(item)) {
            return (isLeave = false);
          }
        });
        if (event.relatedTarget.classList.length == 0) {
          isLeave = false;
        }
        if (isLeave) {
          this.$nextTick(() => {
            if (this.$refs.descPopoverRef) {
              this.$refs.descPopoverRef.hide();
            }
            setTimeout(() => {
              this.mouseInner = false;
            }, 100);
          });
        }
      }else{
        this.mouseInner = false;
      }
    },
  },
  mounted() {
    this.fetchData();
    this.$nextTick(() => {
      if (this.$refs.cardRef && window.ResizeObserver) {
        this.cardResizeObserver = new ResizeObserver(() => {
          this.showImage = this.$refs.cardRef.offsetWidth >= 260;
        });
        this.cardResizeObserver.observe(this.$refs.cardRef);
        this.showImage = this.$refs.cardRef.offsetWidth >= 260;
      }
    });
  },
  beforeUnmount() {
    if (this.cardResizeObserver && this.$refs.cardRef) {
      this.cardResizeObserver.disconnect();
    }
  },
};
</script>
<template>
  <div
    class="card h-100"
    style="cursor: pointer"
    @click="enlarge"
    @mouseenter="mouseInner = true"
    @mouseleave="handleMouseLeave"
    ref="cardRef"
  >
    <div class="card-body d-flex justify-content-between align-items-center">
      <div>
        <div>
          <div class="stat-title mb-2">
            <span>员工平均工作年限</span>
            <el-popover
              placement="bottom-start"
              title="员工平均工作年限"
              :width="350"
              trigger="hover"
              :content="cardDesc"
              ref="descPopoverRef"
            >
              <template #reference>
                <i v-show="cardDesc&&mouseInner" class="bx bx-help-circle" style="font-size: 16px;cursor: pointer;"></i>
              </template>
            </el-popover>
          </div>
          <div class="total mb-2">{{ formatDuration(cardData.number1) }}</div>
        </div>
        <div class="d-flex align-items-center gap-1">
          <span class="text-muted" style="white-space:nowrap">同比</span>
          <span
            :style="{
              color: String(cardData.number2).includes('+')
                ? '#f46a6a'
                : '#34c38f',
              whiteSpace:'nowrap'
            }"
            ><span v-if="String(cardData.number2).includes('+')">+</span
            ><span v-if="String(cardData.number2).includes('-')">-</span
            >{{ formatDuration(cardData.number2) }} ({{
              cardData.number3
            }})</span
          >
          <i
            class="icon iconfont icon-shangzhang"
            style="color: #f46a6a"
            v-if="String(cardData.number2).includes('+') && showImage "
          ></i>
          <i
            class="icon iconfont icon-xiadie"
            style="color: #34c38f"
            v-if="String(cardData.number2).includes('-') && showImage"
          ></i>
        </div>
      </div>
      <div style="position: fixed; right: 20px" v-if="showImage">
        <img
          src="@/assets/images/dashboards/workyear.png"
          style="width: 72px"
        />
      </div>
    </div>
  </div>
  <ConfigContainer
    v-if="isEnlarge"
    :defaultPanelId="4035"
    :v-model:isEnlarge="isEnlarge"
    :close="close"
    :onlyShowModal="true"
    chartTitle="司龄分布"
    groupName="人力资源"
  />
</template>
<style scoped lang="scss">
.stat-title {
  font-weight: bold;
  font-size: 15px;
  display: flex;
  align-items: center;
  gap: 4px;
}
.total {
  font-weight: bold;
  font-size: 16px;
  color: #000000;
}
</style>
