<!-- 假期汇总表列表页，负责按日期范围分页查询员工假期使用汇总并支持导出。 -->
<script setup>
...
import {
  exportLeaveReportSummary,
  queryLeaveReportSummaryPage,
} from "@/api/attendance";
...
const bussId = 471;
const gridName = "leaveReportSummaryGrid";
...
const normalizeRecord = (item = {}, index = 0) => ({
  ...item,
  id: item.id || `${item.talentCode || "talent"}-${index}`,
  sid: (listQuery.value.pageNo - 1) * listQuery.value.pageSize + index + 1,
});

const fetchLeaveReportSummaryList = () => {
  queryLeaveReportSummaryPage(buildQueryParams(), { isLoading: true })
    .then((res) => {
...
const handleExport = (command) => {
  ...
  exportLeaveReportSummary(payload, { isLoading: true }).then((res) => {
    const filePath = res?.data?.filePath;
    const fileName = res?.data?.fileName || "假期汇总表.xlsx";
    if (!filePath) {
          ...
          >
            <div class="d-flex align-items-center">
              <span class="mb-0 flex-grow-1">
                <div
                  class="d-flex leave-report-summary__toolbar"
                  style="gap: 10px"
                >
                  <el-input
                    v-model="keyword"
                  ...
                  <el-date-picker
                    ...
                    class="leave-report-summary__date-range"
                    @change="handleDateRangeChange"
                  />
                  <el-cascader
                    v-model="deptCodes"
                    class="leave-report-summary__cascader"
...
.leave-report-summary__toolbar {
  flex-wrap: wrap;
}

:deep(.leave-report-summary__cascader.el-cascader) {
  width: 260px;
}

:deep(.leave-report-summary__date-range.el-date-editor--daterange) {
